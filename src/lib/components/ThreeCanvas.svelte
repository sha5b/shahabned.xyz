...
import { createPostProcessing } from '$lib/utils/three/postProcessing';
...
function initializeGrid(items, categories, title, pageType) {
    ...
    const onClickHandlers = {
        work: (work) => {
            ...
            goto(`/${work.expand?.category?.title}/${work.title}`);
        },
        category: (category) => {
            disposeGridContainer();
            goto(`/${category.title}`);
    ...
    if (pageType === 'landing') {
        onClickHandlers.work = (work) => {
            disposeGridContainer();
            goto(`/${work.expand?.category?.title}`);
        };
    }
...
onMount(() => {
    ...
    if (pageType === 'work' && work) {
        items = [
            ...
            ...work.gallery.map((item) => ({ id: work.id, thump: item }))
        ];
    }
