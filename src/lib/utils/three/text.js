...
export function createWorkDetailTextTexture(work, width = 640, height = 1024, fontSize = 18, color = 'black') {
    const canvas = document.createElement('canvas');
    canvas.width = width;
    canvas.height = height;
    const context = canvas.getContext('2d');
    ...
    if (work.title) {
        drawText(context, `Title: ${work.title}`, 10, y, width - 20, fontSize, 'left', 'top');
        y += paddingBetweenLines;
    }
    if (work.date) {
        drawText(context, `Date: ${work.date}`, 10, y, width - 20, fontSize, 'left', 'top');
        y += paddingBetweenLines;
    }
    if (work.type) {
        drawText(context, `Type: ${work.type}`, 10, y, width - 20, fontSize, 'left', 'top');
        y += paddingBetweenLines;
    }
    if (work.format) {
        drawText(context, `Format: ${work.format}`, 10, y, width - 20, fontSize, 'left', 'top');
        y += paddingBetweenLines;
    }
    if (work.edition) {
